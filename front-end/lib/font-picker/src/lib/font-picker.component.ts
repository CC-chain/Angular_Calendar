...
export class FontPickerComponent implements OnInit {
    ...
    fpUploadButton: boolean, fpUploadButtonText: string, fpUploadButtonClass: string): void
  {
    ...
    this.service.getAllFonts('popularity').subscribe((fonts: GoogleFontsInterface | null) => {
      this.loading = false;
  ...
  private findFont(searchVal : any, exactMatch: boolean = false): Font {
    return this.findFonts(searchVal, exactMatch)[0];
  }

  private findFonts(searchVal : any, exactMatch: boolean = false): Font[] {
  ...
  private createDialogBox(element : any, offset : any): {top: number, left: number, width: number, height: number} {
    return {
